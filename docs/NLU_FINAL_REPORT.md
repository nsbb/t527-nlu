...
| 소스 | 개수 | 설명 |
|------|------|------|
| Amazon MASSIVE 한국어 | ~3,300 | 공공 데이터, 15개 intent 매핑 |
| kochat | ~8,700 | 공공 데이터, weather_info + dust_query |
| 자체 생성 (94개 intent 커버) | ~200 | 테스트와 겹치지 않는 표현 |
| **총** | **~12,200** | 테스트(르엘)와 겹침 0% |

**원칙:** 학습은 공공 데이터셋(MASSIVE, kochat) 위주. 르엘 시나리오는 테스트로만 사용.
