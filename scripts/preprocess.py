...
def kr_num_to_arabic(text):
    ...
    # 시각 표현: "오전/오후 일곱시" → "오전/오후 7시", "네시" → "4시" 등
    _time_map = {
    ...
    def _replace_time(m):
        word = m.group(1)
        return _time_map.get(word, word) + '시'
    text = re.sub(
        r'(열두|열한|열|아홉|여덟|일곱|여섯|다섯|네|세|두|한)\s*시(?!\s*간)',
        _replace_time, text
    )

    # "이십삼도" / "이십삼 도" → "23도"
    m = re.search(r'(이십|삼십|사십|십)([일이삼사오육칠팔구])?\s*도', text)
    if m:
    ...
    # "스물다섯도" / "스물다섯 도" → "25도"
    m = re.search(r'(스물|서른|마흔|쉰)(하나|둘|셋|넷|다섯|여섯|일곱|여덟|아홉)?\s*도', text)
    if m:
...
STT_CORRECTION = {
    ...
    '온돌': '난방',
    'ventilation': '환기',  # 영어 코드스위칭 → 환기
    '환풍기': '환기',  # 환풍기 먼저 처리 (환풍→환기 전에) — "환기기" 생성 방지
    '환풍': '환기',
    ...
    '도어록': '도어락',
    '도얼락': '도어락',
    '거이': '거실',   # STT 오인식 (거실 → 거이, ㄹ 탈락)
    '거실불': '거실 불',
    '안방불': '안방 불',
    '주방불': '주방 불',
    # 구어체 STT 줄임 (불끄고자 = 불 끄고 자)
    '불끄고자': '불 꺼줘',
    '불켜고자': '불 켜줘',
    '끄고자': '꺼줘',
    '에어컨켜': '에어컨 켜',
    ...
    '다꺼줘': '전체 꺼줘',
    '다켜줘': '전체 켜줘',
    # STT 종결어미 오인식: 켜줘→켜조, 꺼줘→꺼조 (실제로 없는 단어라 안전)
    '켜조': '켜줘',
    '꺼조': '꺼줘',
    # 주의: '켜져'/'꺼져'는 상태 표현("켜져 있어")과 겹쳐서 교체 불가
    # v111: 에어캔 → 에어컨 STT 오인식
    '에어캔': '에어컨',
    # v114: 커튼 STT 오인식 확장
    '커트': '커튼',
    '커텐': '커튼',
    '커텐스': '커튼',
    # v114: 환기 STT 오인식 (외래어 발음)
    '벤틸': '환기',
    '벤틸레이터': '환기',
    '벤틸레이션': '환기',
    # v117: 난방/에어컨 STT 오인식 확장
    '남바': '난방',
    '에어껀': '에어컨',
    '에어컬': '에어컨',
    '에어컹': '에어컨',
    # v118: 환기 STT 오인식 확장 (헌기, 한기-환기 혼동 방지)
    '헌기': '환기',
    '환끼': '환기',
    # v123: 에어컨/보일러 STT 오인식
    '에어킁': '에어컨',
    '보이라': '보일러',
}
