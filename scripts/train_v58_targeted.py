#!/usr/bin/env python3
"""v58 — Targeted augmentation for v46 TS errors + mixup
313 targeted augmentations for the 204 Test Suite errors,
merged into v43 training data, trained with same recipe as v46.
"""
...
def train():
    ...
    train_data = base_data + fix_data * 3
    ...
    for epoch in range(40):
        ...
        if epoch >= 20 and (epoch + 1) % 5 == 0:
            ...
            if ts_combo > best_ts_combo:
                ...
                }, 'checkpoints/cnn_multihead_v58.pt')
                ts_str += " ★"
        ...
        if combo_acc > best_combo:
            ...
            if best_ts_combo == 0:  # Save early best before TS eval starts
                torch.save({
                    'epoch': epoch + 1, 'state': model.state_dict(),
                    'combo': combo_acc, 'fn': fn_acc,
                }, 'checkpoints/cnn_multihead_v58.pt')
            print(f"  ★ Val best: combo={combo_acc:.1f}%, fn={fn_acc:.1f}%")

    # Final comprehensive eval
    ckpt = torch.load('checkpoints/cnn_multihead_v58.pt', map_location='cpu', weights_only=False)
