...
SPECIFIC_PATTERNS = [
    ...
    # 미세먼지 (외출/창문 judgment)
    (r'창문\s*열어도|환기\s*해도\s*돼',
     '현재 미세먼지 수준을 확인하여 환기 가능 여부를 안내드립니다.'),
    (r'공기\s*안\s*좋|미세먼지\s*어때|미세먼지\s*나빠',
    ...
    # 에너지 세부
    (r'에너지\s*사용량\s*어때|에너지\s*어때',
     '최근 3개월 간 에너지 사용량이 감소하고 있습니다.'),
    (r'가장\s*많이\s*쓴\s*달|사용량.*가장\s*(?:많은|높)',
     '최근 3개월 중 8월 사용량이 가장 높습니다.'),
    (r'작년\s*(?:이랑|하고)\s*비교|작년.*대비',
     '지난해와 비슷한 수준으로 사용 중입니다.'),
    (r'(?:사용량\s*)?목표\s*설정',
    ...
    (r'주식\s*(?:어때|시세|현황)|주가\s*알려',
     '오늘 코스피 00.00p, 코스닥 00.00p으로 마감되었습니다.'),
    (r'환율|달러\s*시세',
     '오늘 원/달러 환율은 1,00원 수준입니다.'),

    # 유가
    (r'기름값|유가|휘발유',
     '현재 전국 평균 휘발유 가격은 리터당 00원 수준입니다.'),

    # 의료
    (r'근처\s*병원|병원\s*추천|응급실',
     '가까운 병원 정보를 안내합니다. 응급 상황인 경우 119에 연락하세요.'),
    (r'약국\s*어디|약국\s*찾',
     '근처 24시간 운영 약국 정보를 안내합니다.'),
...
def query_response(fn, room, raw_text):
    ...
    if fn == 'weather_query':
        ...
        if re.search(r'주말|내일|모레', raw_text):
            return '주말 OO 날씨는 최고 00도, 맑은 날씨가 예상됩니다.'
        if re.search(r'바람', raw_text):
            return '오늘 OO 지역에 다소 강한 바람이 예상됩니다.'
        if re.search(r'비\s*오|비\s*와|강수', raw_text):
    ...
    if fn == 'news_query':
        if re.search(r'경제|정치|사회|스포츠|연예|세계', raw_text):
            return '오늘 네이버 뉴스 검색 결과입니다. (해당 주제 뉴스 3건을 요약 브리핑)'
        return '오늘 네이버 뉴스 검색 결과입니다. (주요 뉴스 3건을 요약 브리핑)'
    ...
    # Market
    if fn == 'market_query':
        if re.search(r'주가|주식', raw_text):
            return '오늘 코스피 00.00p, 코스닥 00.00p으로 마감되었습니다.'
        if re.search(r'유가|기름값', raw_text):
            return '현재 전국 평균 휘발유 가격은 리터당 00원 수준입니다.'
        if re.search(r'환율|달러', raw_text):
            return '오늘 원/달러 환율은 00원 수준입니다.'
