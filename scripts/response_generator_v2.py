...
SPECIFIC_PATTERNS = [
    ...
    # 취침 모드 실행 (light_control/schedule 오분류 방지 — 루틴보다 먼저)
    # "설정" 제외 — "취침모드 설정된/예약 어떻게?" 는 조명 예약 쿼리 패턴이 처리
    (r'취침\s*모드\s*(?:실행|켜|작동|시작)(?:\s*해\s*줘|\s*해주세요)?',
     '네, 취침 모드를 실행합니다. 전체 조명을 끄고 난방을 수면 온도로 설정합니다.'),
    (r'잠자리\s*(?:준비|모드)|수면\s*준비\s*해줘',
     '네, 수면 모드로 전환합니다. 조명을 끄고 실내 온도를 수면에 적합하게 설정합니다.'),
    # 자기 전에 다 꺼줘 / 집에 있는 거 다 꺼줘 (굿나잇/전체 오프)
    (r'자기\s*전에?\s*(?:다|전부|모두|전체)\s*꺼|자기\s*전에?\s*꺼줘',
     '네, 전체 조명·난방·에어컨을 끕니다. 편안한 밤 되세요.'),
    # "집에 있는 거 다 꺼줘" 같이 '집에' 명시할 때만 전체 오프 (단순 "다 꺼줘"는 light_control 처리)
    (r'집에?\s*(?:있는\s*거?|있는\s*것|다\s*있는\s*거?)\s*(?:다|전부|모두)\s*(?:꺼|끄)',
     '네, 전체 조명·난방·에어컨·환기 시스템을 끕니다.'),
    (r'굿나잇|잘\s*자|안녕히\s*주무세요|자러\s*가',
     '편안한 밤 되세요. 필요하시면 언제든 불러주세요.'),
    # 대화 종료/취소 (단독 "취소"는 일반 취소 의도)
    (r'^(?:취소|그만|됐어|됐습니다|알았어|알겠습니다|괜찮아|괜찮습니다|필요\s*없어|됩니다|아니야|아니요|아니에요|실수였어|잘못\s*눌렀어|그냥|모르겠어)\s*[\!\.\?]*$',
    ...
    # 잘 못 들었어 / 다시 요청
    # 방금 한 거 취소 (undo 요청)
    (r'방금\s*(?:한\s*거|것\s*좀)?\s*취소|마지막\s*명령\s*취소|되돌려줘',
     '죄송합니다, 마지막 명령 취소는 지원하지 않습니다. 원하시는 동작을 다시 말씀해주세요.'),
    ...
    # ──────────────────────────────────────────
    # 추위 비유 → 난방 켜기
    (r'얼어\s*죽겠|얼어\s*죽을|몸이\s*꽁꽁|손발이\s*꽁꽁|이가\s*딱딱|덜덜\s*떨|온몸이\s*떨|한기가\s*(?:도네|느껴|돌아|나|왔)|냉동실\s*같|냉장고\s*같|시베리아\s*같',
     '많이 추우시군요. 난방을 켜드릴게요.'),
    (r'이불\s*밖이\s*너무\s*추워|이불\s*속에서\s*못\s*나',
     '많이 추우시겠네요. 실내 온도를 높여드릴게요.'),
    # 더위 비유 → 에어컨 켜기
    (r'쪄\s*죽겠|쪄\s*죽을|사우나|찜통|용광로\s*같|화덕\s*(?:같|속)|가마솥\s*같|온실\s*(?:같|이야)',
     '많이 더우시군요. 에어컨을 켜드릴게요.'),
    (r'땀이\s*(?:뻘뻘|철철|줄줄|멈추질\s*않|범벅)|땀에\s*(?:흠뻑|달라붙)',
     '더우신 것 같네요. 에어컨을 켜드릴게요.'),
    (r'더위\s*먹겠|더위\s*먹을|열사병\s*걸릴|기력이\s*다\s*빠져\s*더워',
     '많이 더우시군요. 에어컨을 켜드릴게요.'),
    # 어두움 비유 → 조명 켜기
    (r'동굴\s*같|암흑\s*(?:이야|이네|같)|칠흑\s*같|앞이\s*안\s*보여|앞을\s*못\s*보겠',
     '어두우시군요. 조명을 켜드릴게요.'),
    (r'어두워서\s*(?:발에\s*걸|뭔가에\s*부딪|다닐\s*수|불편)',
     '어두우셨군요. 조명을 켜드릴게요.'),
    # 눈부심 비유 → 조명 끄기/낮추기
    (r'눈이\s*(?:너무\s*)?(?:부셔|부시다|부십니다|따가워|아파|시려|피로해)|눈을\s*못\s*뜨|눈이\s*멀겠',
     '눈이 많이 부시시군요. 조명을 낮춰드릴게요.'),
    # 공기/냄새 비유 → 환기
    (r'공기가\s*(?:탁|나빠|죽어|무거워|텁텁|신선하지)',
     '공기가 좋지 않으시군요. 환기시스템을 켜드릴게요.'),
    (r'머리가\s*(?:띵|무거워|아파\s*(?:환기|공기))|두통이\s*(?:생기|나)',
     '공기가 좋지 않아 두통이 생기시는 것 같네요. 환기시스템을 켜드릴게요.'),
    (r'밀폐된\s*것\s*같|바람이\s*안\s*통|빵빵해',
     '환기가 필요하시군요. 환기시스템을 켜드릴게요.'),
    # 첫 인사/반가움
    (r'처음\s*뵙|반가워|처음\s*만나|첫\s*인사',
     '반갑습니다! 저는 원더입니다. 집 안 기기 제어, 정보 조회 등 무엇이든 말씀해주세요.'),
    # AI 기분 질의 (상태는 제외 — "집 상태 어때" 등과 충돌)
    (r'(?:기분|컨디션)\s*(?:이\s*)?(?:어때|어떠|괜찮)',
     '저는 항상 좋습니다! 무엇을 도와드릴까요?'),
    # 심심함/무료함 → 기능 소개
    (r'심심해?|무료해?|심심한데\s*뭐|할\s*게\s*없어?',
     '저는 조명, 난방, 에어컨, 환기, 날씨, 에너지 등을 도와드릴 수 있어요. 필요한 것을 말씀해주세요.'),
    ...
    # 긍정 피드백 (잘 됐어 / 완벽해 → 긍정 응답)
    (r'^(?:잘\s*됐어?|완벽해?|훌륭해?|최고야?|대단해|좋아|오케이|ok|OK)\s*[\!\.]*$',
     '다행이에요. 언제든 불러주세요.'),
    # 알겠어/이해했어 → 확인 응답
    (r'^(?:알겠어|알겠습니다|이해했어|이해했습니다|알았어|알았습니다)\s*[\!\.]*$',
     '네, 알겠습니다. 더 필요하신 게 있으시면 말씀해주세요.'),
    # 주목 호칭 (이봐요/저기요 → 대기 응답)
    (r'^(?:이봐요?|저기요?|야|헤이|hey)\s*[\!\.?]*$',
     '네, 말씀해주세요.'),
    # 시스템 무반응/오류 (security_mode/light_control 오분류 방지)
    (r'반응\s*(?:이\s*)?(?:없어|안\s*해|안\s*해요)|작동\s*(?:이\s*)?(?:안\s*해|안\s*돼)|못\s*알아듣',
     '죄송합니다. 월패드를 재시작하거나 관리사무소에 문의해주세요.'),
    # 현재 켜져 있는 기기 (schedule 오분류 방지)
    (r'현재\s*(?:켜져\s*있는|작동\s*중인|동작\s*중인)\s*기기|지금\s*켜져\s*있는\s*(?:기기|기기들)',
     '현재 각실 조명과 난방이 켜져 있고 실내 온도는 00도 입니다. 자세한 상태는 홈 화면에서 확인하세요.'),
    # 천천히/잠깐/기다려 → 대기 응답
    (r'(?:잠깐|잠시)\s*(?:기다려|기다려줘|기다리세요|만요?)',
     '알겠습니다. 준비되시면 말씀해주세요.'),
    (r'^(?:천천히|잠깐|잠깐만|잠시|잠시만|조금만\s*있다가)\s*(?:해줘|해주세요|요)?\s*$',
     '알겠습니다. 준비되시면 말씀해주세요.'),
    # 그만해줘 → 중단 응답
    (r'^(?:그만|됐어|됐습니다|닥쳐|조용|그만해줘|됐으니)\s*(?:해줘|해주세요|요?)?\s*$',
     '네, 알겠습니다. 필요하시면 언제든 불러주세요.'),
    # 긍정 감정 표현 — light_control/heat_control 오분류 방지
    (r'집(?:이|에서)?\s*(?:좋다|편하다|좋아|마음에\s*들어)',
     '좋은 집 환경을 유지하도록 도와드릴게요. 필요하시면 말씀해주세요.'),
    (r'따뜻(?:해서|하니|하고|한데)\s*좋(?:아|네|다)|포근해서\s*좋',
     '따뜻하게 지내시길 바랍니다. 실내 온도 조절이 필요하시면 말씀해주세요.'),
    # 아이 공부/수면 맥락 상황 → 맥락 인식
    (r'아이\s*(?:가|이|는|도)?\s*공부\s*(?:중|하고|해)',
     '공부하는 시간에는 집중 조명(밝은 주광색)이 도움이 됩니다. 조명을 조절해드릴까요?'),
    (r'아기\s*(?:가|는|도)?\s*(?:자|잠든|잠들|자고)',
     '아기가 잘 자도록 조명을 어둡게 하고 조용하게 유지할게요.'),
    # 약 복용 알림 → 알람 기능 안내
    (r'약\s*(?:먹을\s*시간|먹으라고|알려줘|알람)',
     '약 복용 알림은 스케줄 메뉴에서 알람 시간을 설정해 받을 수 있습니다.'),
    ...
    # 119/응급 연결
    (r'119\s*(?:연결|전화|신고|불러)|구급대\s*(?:불러|연결|신고)',
     '119에 연결합니다. 긴급하면 직접 119에 전화하세요.'),
    # 가스 새다 (가스 냄새와 별개 — "새다/누출" 키워드)
    (r'가스\s*(?:가\s*)?새|가스\s*누출|가스\s*(?:가\s*)?나오',
     '⚠️ 즉시 환기하고 가스 밸브를 잠근 후 119에 연락하세요.'),
    # 물 새다/수도 문제
    (r'물\s*(?:이\s*)?새|누수\s*(?:됐|발생|있어)|물\s*새고|천장\s*(?:에서\s*)?물|물\s*(?:이\s*)?뚝뚝',
     '누수 발생 시 즉시 수도 밸브를 잠그고 관리사무소에 신고해주세요.'),
    (r'수돗물\s*(?:이\s*)?(?:안\s*나|없어|단수|끊겼|안\s*나와)|수도\s*(?:가\s*)?(?:안\s*나오|막혀|끊겨)|단수',
     '단수 문제는 관리사무소 또는 수도사업소(120)에 신고해주세요.'),
    ...
    (r'(?:조명|불|전등)\s*(?:이?\s*)?(?:계속|자꾸)\s*(?:꺼졌다\s*켜졌다|켜졌다\s*꺼졌다|꺼졌다가\s*켜|깜빡)',
     '조명 점멸 현상은 전구 수명이 다 됐거나 배선 문제일 수 있습니다. 관리사무소에 점검을 요청해주세요.'),
    # 눈 부심 / 조명 너무 밝음 → 밝기 낮추기 (light_control dir=on 오분류 방지)
    (r'(?:불|조명|전등)\s*(?:이?\s*)?너무\s*밝(?:아|아서|아요)|눈\s*(?:이?\s*)?(?:아파|부셔|부셔서|시려)',
     '네, 조명 밝기를 낮추겠습니다.'),
    # 월패드 소리 이상 (home_info 오분류 방지)
    (r'월패드\s*(?:소리|음량|볼륨)\s*(?:안\s*나|없어|안\s*들려|작아)',
     '월패드 볼륨을 확인해주세요. 설정 → 소리에서 조절하거나 재시작해보세요. 지속되면 관리사무소에 문의해주세요.'),
    # 월패드 카메라 (home_info 오분류 방지)
    (r'월패드\s*(?:카메라|영상|화면)\s*(?:보여|봐|켜|확인)',
     '월패드 카메라 영상은 화면 하단 카메라 버튼을 눌러 확인하실 수 있습니다.'),
    # 아파트 행정 심화
    (r'이사\s*(?:나가|퇴거|떠나|나오)',
    ...
    (r'태풍\s*(?:경보|예보|주의보|있어|올\s*것|온다|온대|오나)',
     '현재 태풍 관련 기상 특보를 확인해주세요. 행정안전부 재난안전정보에서 확인하실 수 있습니다.'),
    # 미세먼지 경보 해제 (security_mode 오분류 방지)
    (r'미세먼지\s*(?:경보|주의보|비상저감조치)\s*(?:해제|끝났|종료)',
     '미세먼지 경보가 해제되었는지는 에어코리아(airkorea.or.kr) 또는 날씨 앱에서 확인해주세요.'),
    ...
    (r'뭐\s*(?:먹|드|식사|요리)(?:으면|을까|어야|고\s*싶|고\s*싶어|을|지|을지|까)?|오늘\s*뭐\s*(?:먹|드)|저녁\s*뭐|점심\s*뭐|아침\s*뭐',
     '음식 메뉴 추천은 지원하지 않습니다. 배달앱이나 냉장고 식재료를 확인해보세요.'),
    ...
    (r'택배\s*(?:맡아|대신\s*받아|보관\s*해|받아\s*줄)',
     '무인 택배함을 이용하시거나 관리사무소에 택배 대행 서비스를 문의해주세요.'),
    (r'택배\s*보관함',
    ...
    (r'(?:헬스장|피트니스\s*센터|피트니스)\s*(?:몇\s*시|이용\s*시간|이용\s*방법|운영|언제|어떻게|가능|있어|예약)',
     '헬스장 운영시간은 오전 6시~오후 10시입니다. 예약은 입주민 앱에서 가능합니다.'),
    (r'수영장\s*(?:이용|운영|몇\s*시|언제|가능|있어|있나)',
     '수영장 운영시간 및 예약은 입주민 앱 또는 관리사무소에서 확인해주세요.'),
    (r'골프\s*(?:연습장|레인지)\s*(?:예약|이용|몇\s*시)',
     '골프 연습장 예약은 입주민 앱에서 가능합니다.'),
    # 골프 연습장/독서실 존재 쿼리 (있어/없어 등)
    (r'골프\s*(?:연습장|레인지)\s*(?:있어|있나|어디|위치)',
     '골프 연습장 운영시간 및 예약은 입주민 앱 또는 관리사무소에서 확인해주세요.'),
    (r'독서실\s*(?:몇\s*시|이용\s*시간|운영|언제|있어|있나|사용|가능|어떻게)',
     '독서실 운영시간은 관리사무소 또는 입주민 앱에서 확인해주세요.'),
    (r'스카이\s*라운지\s*(?:이용|운영|몇\s*시|어떻게|어디)',
     '스카이라운지 이용 시간 및 예약은 입주민 앱 또는 관리사무소에서 확인해주세요.'),
    (r'커뮤니티\s*센터\s*(?:이용|운영|몇\s*시|어떻게)',
     '커뮤니티센터 이용 방법은 관리사무소 또는 입주민 앱에서 확인해주세요.'),
    # 어린이집/놀이터 위치 (security_mode/news 오분류 방지)
    (r'어린이집\s*(?:있어|있나|어디|위치)',
     '단지 내 어린이집 정보는 관리사무소에 문의해주세요.'),
    (r'어린이집\s*셔틀|셔틀\s*(?:몇\s*시|시간|언제)',
     '어린이집 셔틀 시간은 관리사무소 또는 어린이집에 직접 문의해주세요.'),
    # 아이 귀가 알림 (home_info dir=on 오분류 방지)
    (r'아이\s*(?:귀가|도착|왔)\s*(?:알림|알려|통보|문자)',
     '아이 귀가 알림은 입주민 앱 또는 관리사무소를 통해 설정하실 수 있습니다.'),
    # 방문객/손님 주차 (vehicle_manage 오분류 방지)
    (r'방문(?:객|자)?\s*주차|손님\s*주차|외부인\s*주차',
     '방문객 주차는 방문 전 관리사무소에 신청하거나 단지 내 방문자 주차 구역을 이용해주세요.'),
    (r'(?:어린이|아이)\s*놀이터\s*(?:어디|위치|있어)',
     '놀이터는 단지 내 조경 구역에 위치합니다. 자세한 위치는 단지 안내도를 확인해주세요.'),
    # 쓰레기 처리 위치 (news_query 오분류 방지)
    (r'쓰레기\s*(?:통|봉투|함)?\s*(?:어디|버리|처리|투기)',
     '쓰레기 분리수거장은 각 동 지하 주차장 입구에 있습니다. 분리수거 수거일은 관리사무소에 문의해주세요.'),
    # 택배함 비밀번호
    (r'택배\s*(?:함|보관함)\s*(?:비밀번호|비번|코드|번호)',
     '택배 보관함 비밀번호는 배송 시 문자로 전송됩니다. 만료 시 관리사무소에 문의해주세요.'),
    # 커튼 타이머/예약
    (r'(?:커튼|블라인드)\s*(?:타이머|예약|스케줄)',
     '전동커튼 타이머 설정은 월패드 일정 메뉴에서 가능합니다.'),
    ...
    # 에어프라이어/전자레인지/로봇청소기 → 미지원
    (r'에어프라이어|전자레인지|오븐',
     '에어프라이어·전자레인지 제어는 지원하지 않는 기능입니다.'),
    (r'로봇\s*청소기',
     '로봇청소기 제어는 지원하지 않는 기능입니다.'),
    # 전기장판/전기매트 → 미지원 (heat_control 오분류 방지, "전기요금" 오매칭 방지)
    (r'전기\s*(?:장판|매트|담요)|전기\s*요(?!\s*금)',
     '전기장판·전기매트 제어는 지원하지 않는 기능입니다.'),
    # 소화기 위치 — 안전 안내
    (r'소화기\s*(?:어디|위치|찾)',
     '소화기는 현관 옆 또는 부엌에 비치되어 있습니다. 건물 각층 복도에도 소화기가 비치되어 있습니다.'),
    # 피난 경로/비상구 — 안전 안내
    (r'피난\s*(?:경로|통로|방법|어떻게)|비상구\s*(?:어디|위치)',
     '피난 시 계단(비상구)을 이용하고 엘리베이터 사용은 금지입니다. 층간 비상구 표지판을 따라 이동하세요.'),
    ...
    # 음악/미디어 끄기 (미디어 재생 불가 — dir=off 오분류 방지)
    (r'음악\s*(?:꺼|꺼줘|중지|멈춰)|음악\s*그만',
     '죄송합니다. 미디어 재생은 지원하지 않는 기능입니다.'),
    # 스포티파이/멜론/유튜브뮤직 등 특정 앱 (light/ac 오분류 방지)
    (r'스포티파이|멜론|지니|플로\s*뮤직|유튜브\s*뮤직|애플\s*뮤직',
     '죄송합니다. 미디어 앱 실행은 지원하지 않는 기능입니다.'),
    # 라면/요리 도움 요청 (unknown → 안내)
    (r'라면\s*(?:끓여|만들어|해줘)|요리\s*(?:좀\s*)?도와줘|밥\s*(?:해줘|만들어줘)',
     '요리 도움은 지원하지 않습니다. 조리 환경 최적화(환기, 온도)는 도와드릴 수 있어요.'),
    # 콘센트/스마트플러그 (light_control 오분류 방지)
    (r'콘센트|스마트플러그|스마트\s*플러그',
     '스마트 플러그 제어는 연결된 기기에서 직접 조절해주세요.'),
    # 현관/실내 카메라 제어 → CCTV 안내
    (r'(?:현관|실내|집\s*안)\s*카메라\s*(?:켜|봐|보여|확인)',
     'CCTV 영상은 월패드 화면에서 확인하실 수 있습니다.'),
    # 이사/청소/인테리어 외부업체 → 미지원
    (r'이사\s*(?:청소|업체)|청소\s*업체\s*(?:연결|알려|신청)',
     '외부 청소 업체 연결은 지원하지 않습니다. 관리사무소에 문의해주세요.'),
    (r'인테리어\s*업체\s*(?:알려|연결|추천)',
     '인테리어 업체 정보는 지원하지 않습니다. 관리사무소에 문의해주세요.'),
    # 집수리 → 하자 신고
    (r'집수리\s*(?:신청|해줘|하고\s*싶어)|하자\s*(?:신고|접수|수리)',
     '하자 신고는 입주민 앱 또는 관리사무소에 접수해주세요.'),
    # 개인정보/녹음/음성 기록 → 프라이버시 안내
    (r'개인정보\s*(?:보호|설정|어디)',
     '개인정보 보호 설정은 월패드 환경설정 > 개인정보 메뉴에서 확인하실 수 있습니다.'),
    (r'녹음\s*(?:되고|중|하고)\s*있|음성\s*(?:기록|저장|녹음)',
     '음성 명령은 처리 후 저장되지 않습니다. 개인정보 설정은 환경설정 메뉴에서 확인하세요.'),
    # 경비실/관리사무소 번호 문의 (연결 명령보다 먼저)
    (r'경비실\s*(?:번호|전화번호|연락처|몇\s*번|뭐야)',
     '경비실 전화번호는 월패드 화면 → 연락처에서 확인하실 수 있습니다.'),
    (r'관리사무소\s*(?:번호|전화번호|연락처|몇\s*번|뭐야)',
     '관리사무소 연락처는 000-0000-0000 입니다.'),
    (r'입주(?:자|민)\s*(?:대표|대표회의)\s*(?:연락처|번호|전화)',
     '입주자 대표 연락처는 관리사무소에 문의해주세요.'),
    # 전기 누진세 (weather_query 오분류 방지)
    (r'누진세|전기\s*누진\s*(?:요금|세)',
     '전기 누진세는 사용량 구간에 따라 단가가 높아집니다. 이번 달 사용량은 월패드 에너지 메뉴에서 확인하세요.'),
    # 경비원/경비실 호출
    (r'경비원\s*(?:불러|호출|연결|전화)|경비\s*(?:불러|호출)',
     '경비실로 통화를 연결합니다.'),
    # 응급 전화번호 연결
    (r'112\s*(?:연결|전화|신고|불러)|경찰\s*(?:연결|신고|불러)',
     '112에 연결합니다. 긴급 상황이 아닌 경우 월패드 비상 버튼을 이용해주세요.'),
    # 경비실/로비/관리사무소 인터컴
    (r'경비실\s*(?:연결|통화|전화|불러)',
     '경비실로 통화를 연결합니다.'),
    (r'로비\s*(?:통화|연결|전화)',
     '로비로 통화를 연결합니다.'),
    # 인터폰 볼륨 (볼륨 조절 먼저, 그 다음 일반 인터폰 응답)
    (r'인터폰\s*(?:소리|볼륨|음량)\s*(?:높여|올려|크게)',
     '인터폰 벨 소리는 월패드 화면 → 설정 → 소리에서 조절하실 수 있습니다.'),
    (r'인터폰\s*(?:소리|볼륨|음량)\s*(?:낮춰|줄여|작게)',
     '인터폰 벨 소리는 월패드 화면 → 설정 → 소리에서 조절하실 수 있습니다.'),
    (r'인터폰|인터컴',
    ...
    # 도어락 비밀번호 분실/잊어버림
    (r'도어락\s*(?:비번|비밀번호|패스워드)\s*(?:잊어버|잊어버렸|모르겠|몰라)|핀\s*(?:번호|코드)\s*(?:잊어|몰라|틀렸)',
     '도어락 비밀번호를 분실하셨나요? 관리사무소에 문의하거나 도어락 제조사 A/S 센터를 통해 초기화 가능합니다.'),
    # 임시 비밀번호 발급
    (r'임시\s*(?:비밀번호|비번|코드|패스)\s*(?:만들어|발급|생성|주세요)',
     '임시 비밀번호 발급은 월패드 화면 → 도어락 설정 → 임시 비밀번호에서 설정 가능합니다.'),
    # 방문객 출입 기록 조회
    ...
    # 조명 흰색/주광색 → 차가운 색온도
    (r'조명\s*(?:색\s*)?(?:흰색|하얀색|주광색|백색)\s*(?:으로|해줘)|조명\s*하얗게',
     '네, 조명을 차가운 흰색(주광색)으로 설정합니다.'),
    # 조명 노란색/전구색 → 따뜻한 색온도
    (r'조명\s*(?:색\s*)?(?:노란색|황색|전구색|오렌지)\s*(?:으로|해줘)',
     '네, 조명을 따뜻한 노란색(전구색)으로 설정합니다.'),
    # 조명 반으로 줄여줘 → 50%
    (r'(?:조명|불)\s*(?:반으로|반만)\s*(?:줄여|낮춰|해줘|내려)',
     '네, 조명을 50% 밝기로 조절합니다.'),
    ...
    # 전기 절약 방법 안내
    (r'전기\s*(?:아끼|아끼는|절약|절약하는|절전)\s*(?:방법|법|어떻|팁|어떻게|알려)',
    ...
    # 방문자/배달 도착 여부 쿼리 (home_info/door_control 오분류 방지)
    (r'문\s*앞에?\s*(?:누가|사람이|아무도)\s*(?:있어|있나|있나요)',
     '방문자 카메라 영상은 월패드 화면에서 확인하실 수 있습니다.'),
    (r'방문객\s*(?:있어|있나|왔어|왔나)',
     '방문자 카메라 영상은 월패드 화면에서 확인하실 수 있습니다.'),
    (r'초인종\s*(?:눌렸|눌러|울렸|왔어|왔나)',
     '초인종 기록은 월패드 화면에서 확인하실 수 있습니다. 방문자 영상도 함께 확인해보세요.'),
    (r'배달\s*(?:왔나|왔어|온\s*것|온\s*거)',
     '배달 도착 확인은 공동현관 카메라 또는 초인종 기록으로 확인해보세요.'),
    # 방범 카메라 확인 (security_mode 오분류 방지)
    (r'방범\s*(?:카메라|cctv)\s*(?:확인|봐|보여|켜|점검)',
     'CCTV 영상은 월패드 화면에서 확인하실 수 있습니다.'),
    # 집에 침입자/누가 들어왔어 (security_mode 방범 맥락)
    (r'집에?\s*(?:누가|사람이|도둑이?|침입자가?)\s*(?:들어왔|들어온|침입)',
     '이상 감지 시 즉시 119/112에 연락하세요. CCTV 영상은 월패드 화면에서 확인하실 수 있습니다.'),
    # 재실 여부 쿼리 (없어/없나 형태만 — 있는데는 감정 맥락)
    (r'집에?\s*(?:사람|아무도)\s*(?:없어|없나|없는지)|집\s*안에?\s*아무도\s*없',
     '현재 집 안에 아무도 없는 것으로 확인됩니다. 외출 모드로 전환할까요?'),
    # 외출 전 체크리스트 / 집 비울 때 확인
    (r'외출\s*(?:전에?\s*)?(?:확인|체크|점검|해줘)|나가기\s*전에?\s*(?:확인|체크)',
     '외출 전 체크리스트: 가스 밸브 잠금, 조명 소등, 도어락 확인을 권장합니다.'),
    (r'집\s*(?:비울|비우|비워|나올|떠날|떠나)\s*때|자리\s*비울\s*때',
     '외출 전 체크리스트: 가스 밸브 잠금, 조명 소등, 도어락 확인을 권장합니다. 외출 모드를 설정하시겠어요?'),
    ...
    # 배달 시켜도 돼/피자/음식 주문 판단형 (judge 오분류 방지)
    (r'(?:피자|치킨|짜장면|배달음식|음식)\s*(?:시켜도|주문해도|먹어도)\s*(?:돼|괜찮|될까)',
     '음식 주문은 배달 앱을 이용해주세요. 집 안 기기 제어가 필요하시면 말씀해주세요.'),
    # 건강 기록 (만보/운동/수면) → 미지원
    (r'만보\s*(?:걸었|몇\s*보|걷|달성)',
     '만보계 기능은 지원하지 않습니다. 건강 앱을 이용해주세요.'),
    (r'(?:오늘|이번\s*주)?\s*운동\s*(?:얼마나|기록|몇\s*분|몇\s*칼로리)',
     '운동 기록은 지원하지 않습니다. 건강 앱을 이용해주세요.'),
    (r'수면\s*(?:시간|기록|얼마|몇\s*시간)',
     '수면 기록은 지원하지 않습니다. 건강 앱을 이용해주세요.'),
    # 주기적 알림 (스트레칭/수분/약) — heat_control 오분류 방지
    (r'(?:\d+분|매\s*\d+분|매시간)\s*마다\s*(?:알림|알려|스트레칭|수분|물)',
     '주기적 알림은 월패드 스케줄 메뉴에서 설정 가능합니다.'),
    # 스트레칭/운동 방법 안내 — 지원 불가
    (r'^스트레칭\s*(?:알려줘|방법|어떻게)|스트레칭\s*(?:뭐|무엇)',
     '스트레칭 안내는 지원하지 않습니다. 건강 앱이나 인터넷을 이용해보세요.'),
    # 주차장 자리 있어 (vehicle_manage generic 방지)
    (r'주차장\s*(?:자리|공간|여유|빈자리)\s*(?:있어|있나|있어요)',
     '방문자 주차 공간은 관리사무소에 문의해주시거나 입주민 앱에서 확인해주세요.'),
    (r'택시\s*(?:불러|부를|호출)',
    ...
    # 바깥/외부 온도 (실내 온도 패턴보다 먼저)
    (r'(?:바깥|밖|외부|야외)\s*(?:온도|기온)\s*(?:알려|어때|몇\s*도)',
     '현재 OO구 OO동 기온은 OO도입니다.'),
    # 집 안/실내 온도 (weather_judge 오분류 방지, "온도가 몇 도" 형태 포함)
    (r'집\s*(?:안|내부|실내)?\s*온도.{0,5}(?:몇\s*도|어때|알려)',
     '현재 실내 온도는 23도입니다.'),
    (r'(?:실내|지금|현재)?\s*온도\s*(?:몇\s*도|어때|알려|얼마)',
     '현재 실내 온도는 23도이며 난방 설정은 25도로 되어 있습니다.'),
    # "지금 몇 도야?" — 온도 키워드 없이 바로 묻는 경우
    (r'^지금\s*몇\s*도야\??$|^지금\s*기온\s*(?:몇\s*도|어때)',
     '현재 OO구 OO동 기온은 OO도입니다.'),
    ...
    (r'(?:불|조명)\s*(?:좀\s*)?밝(?:혀|혀줘|혀주세요|게\s*해|게\s*해줘|게\s*해주세요)',
     '네, 조명 밝기를 올립니다.'),
    # 매뉴얼/시스템
    (r'네가\s*할\s*수\s*있|뭘\s*할\s*수\s*있|할\s*수\s*있는\s*일|기능\s*뭐|기능\s*알려|어떤\s*기능|기능들\s*(?:이\s*)?(?:있어|있나)',
     '네, 저는 사용자 요청에 따라 조명 등 집안 기기 상태를 조회하고 조절 및 예약할 수 있어요'),
    (r'음성\s*인식\s*(?:어떻게|어떻|사용법|쓰는|써|방법)|어떻게\s*쓰는\s*거야',
     "음성인식은 '하이 원더'라고 부른 뒤 원하는 내용을 말씀해주세요. 예: '하이 원더, 거실 조명 켜줘'"),
    ...
    (r'단지\s*(?:내|안)?\s*카페|입주민\s*카페\s*(?:있어|있나|어디)',
     '단지 내 카페는 입주민 앱에서 위치와 운영 시간을 확인하실 수 있습니다.'),
    ...
    # 눈 예보 여부 쿼리 (비는 GT 주간예보 시나리오와 충돌 → 제외)
    (r'눈\s*(?:와|올까|오나|예보|내려)',
     '오늘 눈 예보는 없습니다. 날씨 정보를 확인하시려면 날씨 메뉴를 이용해주세요.'),
    # 야외활동 판단형 (등산/낚시/산책/자전거/드라이브 등)
    (r'자전거\s*(?:타|탈|탈까|타도|괜찮|좋은)|드라이브\s*(?:하기|가기|좋은|괜찮)',
     '오늘 OO구 OO동 날씨는 맑고 야외 활동하기 적합합니다.'),
    (r'등산|낚시|조깅|달리기|산책\s*(?:가도|갈까|해도)',
     '오늘 OO구 OO동 날씨는 맑고 야외 활동하기 적합합니다.'),
    # 환기시키기 좋은 날 판단
    (r'환기\s*(?:시키기|하기|해도)\s*(?:좋은|좋아|괜찮|적합)',
     '현재 OO구 OO동 미세먼지는 좋음 수준으로 창문을 열어 환기하기 적절합니다.'),
    # 습함/눅눅함 → 환기/제습 추천
    (r'(?:집이|실내가|방이)?\s*(?:습해|눅눅|습도\s*높|눅눅해|퀴퀴)',
     '실내가 습한 경우 환기 시스템을 켜거나 에어컨 제습 모드를 이용하시면 도움이 됩니다. 환기 시스템을 켜드릴까요?'),
    # 전기세 절약/에너지 효율 목표
    (r'전기세\s*(?:아끼|절약|줄이)|에너지\s*(?:효율|절약)\s*(?:적으로|하게)\s*(?:쓰|사용)',
     '에너지 절약 팁: 사용하지 않는 조명 끄기, 에어컨 온도 1~2도 조정, 대기전력 차단이 효과적입니다.'),
    # 에어컨 가동 시간 쿼리
    (r'에어컨\s*몇\s*시간|에어컨\s*(?:가동|운전)\s*시간',
     '오늘 에어컨 가동 시간은 OO시간입니다.'),
    # 배달 알림 → 미지원
    (r'배달\s*(?:왔을\s*때|오면|알려|알림|연락)',
     '배달 도착 알림은 지원하지 않습니다. 인터폰이나 문자를 확인해주세요.'),
    # 조건부 날씨 제어 (비/눈 올 때 창문 닫아줘 — gas_control 오분류 방지)
    (r'(?:비|눈).{0,15}창문.{0,5}(?:닫|잠)',
     '조건부 자동 제어는 지원하지 않습니다. 날씨 변화 시 창문을 수동으로 닫아주세요.'),
    ...
    (r'빨래\s*(?:너|널|넣|말리|건조)|세탁물\s*(?:너|널|건조)',
     '오늘 날씨상 빨래 널기에 적합합니다.'),
    # 이불/침구 — 취침 온도 맥락
    (r'이불\s*(?:몇\s*장|얼마나|어떻게\s*덮|많이\s*덮|덮어야)',
     '오늘 밤 OO구 OO동 최저 기온은 00도입니다. 이불 1장으로 충분할 것으로 예상됩니다.'),
    # 난방 필요 여부 판단
    (r'(?:요즘|지금|오늘)\s*난방\s*(?:필요|해야|켜야)|난방\s*(?:필요|켜야)\s*할까',
     '오늘 OO구 OO동 최저 기온은 00도입니다. 실내 온도에 따라 난방 가동을 결정해주세요.'),
    ...
    (r'기상했어|일어났어|일어났다|잠에서\s*깼|일어났는데',
     '좋은 아침입니다! 오늘 하루도 좋은 하루 되세요.'),
    # 시스템 업데이트
    (r'업데이트\s*(?:됐|있어|했|해줘|확인)|버전\s*업데이트|최신\s*버전|펌웨어\s*업데이트',
     '월패드 환경설정에서 소프트웨어 업데이트를 확인하실 수 있습니다.'),
    # 앱 버전 조회
    (r'(?:앱|애플리케이션|소프트웨어)?\s*버전\s*(?:이\s*)?(?:몇|어때|알려)',
     '앱 버전 정보는 월패드 환경설정 > 앱 정보에서 확인하실 수 있습니다.'),
    # 저장공간 (vehicle_manage 오분류 방지)
    (r'저장\s*공간|메모리\s*얼마|남은\s*공간',
     '저장공간 정보는 월패드 환경설정 > 시스템 정보에서 확인하실 수 있습니다.'),
    # 월패드 재시작
    (r'월패드\s*(?:재시작|다시\s*시작|리부팅|리셋)',
     '월패드 재시작은 환경설정 > 시스템 설정 > 재시작에서 진행하실 수 있습니다.'),
    # 로그 파일 → 안내
    (r'로그\s*(?:파일|보여|확인|조회)',
     '로그 정보는 월패드 환경설정 > 시스템 정보에서 확인하실 수 있습니다.'),
    # 초기화/리셋 방법 → 설정 안내
    (r'초기화\s*(?:해|해줘|해주세요|하는\s*방법)|공장\s*초기화',
     '초기화는 월패드 환경설정 > 시스템 설정 > 초기화 메뉴에서 진행하실 수 있습니다.'),
    # 홈 루틴 제어 (curtain/light 오분류 방지)
    (r'외출\s*루틴\s*(?:설정|실행|켜|작동)',
     '외출 루틴을 실행합니다. 전체 조명·에어컨을 끄고 외출 모드로 전환합니다.'),
    (r'귀가\s*루틴\s*(?:설정|실행|켜|작동)',
     '귀가 루틴을 실행합니다. 조명과 난방을 적절히 켜고 환기 시스템을 시작합니다.'),
    # 귀가/외출 자동화 설정 (home_info direct 오분류 방지)
    (r'귀가\s*(?:하면|시)\s*자동으로\s*(?:켜지게|켜줘|켤|시작)',
     '귀가 자동화 설정은 월패드 스케줄 또는 귀가 루틴 메뉴에서 가능합니다.'),
    (r'외출\s*(?:하면|시)\s*자동으로\s*(?:꺼지게|꺼줘|끄|소등)',
     '외출 시 자동 소등 설정은 월패드 스케줄 메뉴에서 가능합니다.'),
    # 매일 특정 시간 자동화 (schedule_manage/light_control 오분류 방지)
    (r'매일\s*(?:아침|저녁|밤|오전|오후)?\s*\d+\s*시\s*(?:에\s*)?(?:조명|불)\s*(?:켜줘|켜)',
     '네, 매일 해당 시간에 조명이 켜지도록 설정했습니다.'),
    # 쾌적한 환경 최적화 (system_meta dir=down 오분류 방지)
    (r'쾌적한\s*환경|최적화\s*(?:해줘|설정|모드)|에너지\s*효율\s*(?:높은|설정|모드)',
     '쾌적한 환경을 위해 온도 22~24도, 습도 50%, 환기 활성화를 설정합니다.'),
    (r'기상\s*루틴\s*(?:설정|실행|켜|작동)',
     '기상 루틴을 실행합니다. 거실 조명을 켜고 난방 온도를 올립니다.'),
    (r'취침\s*루틴\s*(?:설정|실행|켜|작동)',
     '취침 루틴을 실행합니다. 전체 조명을 끄고 수면에 적합한 온도로 설정합니다.'),
    # 기기 브랜드/모델 정보
    (r'에어컨\s*(?:브랜드|제조사|모델)',
     '에어컨 모델 정보는 기기 본체 스티커 또는 설명서에서 확인해주세요.'),
    (r'(?:어떤|무슨|어느)\s*월패드|월패드\s*(?:브랜드|모델|제조사|회사)',
     '르엘 월패드를 사용하고 계십니다. 자세한 사양은 설정 > 기기 정보에서 확인하실 수 있습니다.'),
    # 오늘 예약/스케줄 조회
    (r'오늘\s*(?:예약된|예약|스케줄|일정)\s*(?:게|것\s*)?\s*(?:뭐|있어|보여)',
     '오늘 예약된 항목은 월패드 스케줄 메뉴에서 확인하실 수 있습니다.'),
    # 공휴일/달력 쿼리
    ...
    (r'에어컨\s*(?:바람\s*방향|풍향)\s*(?:바꿔|변경|조절)',
     '네, 에어컨 바람 방향을 조절합니다.'),
    # 에어컨 바람 위/아래 방향 (루버 키워드 없을 때 — direct_respond 오분류 방지)
    (r'에어컨\s*(?:바람|풍향)\s*(?:위|위쪽|상|상향|올려)\s*(?:로|해줘|해주세요)?',
     '네, 에어컨 루버를 위쪽으로 조절합니다.'),
    (r'에어컨\s*(?:바람|풍향)\s*(?:아래|아래쪽|하|하향|내려)\s*(?:로|해줘|해주세요)?',
     '네, 에어컨 루버를 아래쪽으로 조절합니다.'),
    ...
    (r'환기\s*(?:시스템\s*)?필터\s*(?:언제\s*교체|교체|교환)|(?:환기\s*)?필터\s*교체',
     '현재 환기 필터 교체 시기가 도래했습니다. 환기 필터 교체 주기는 3~6개월입니다.'),
    ...
    (r'추워서\s*(?:못\s*살|죽겠|힘들)|너무\s*추워\s*(?:못\s*살|죽겠)',
     '네, 난방을 켭니다.'),
    # 색온도 따뜻하게 — 조명 색온도 요청 (반드시 따뜻하게 패턴보다 먼저!)
    (r'(?:조명|불)\s*(?:색|색온도|색깔)?\s*따뜻하게|색온도\s*따뜻하게|전구색\s*으로|따뜻한\s*(?:빛|색온도|조명)',
     '네, 조명을 따뜻한 색온도로 설정합니다.'),
    # 시원하게/따뜻하게 직접 요청 (에어컨/난방 온도 조절)
    (r'(?:좀\s*더\s*)?시원하게\s*(?:해\s*줄?\s*수|할\s*수|해|해줘|해주세요|했으면)',
     '네, 에어컨 온도를 낮추겠습니다.'),
    (r'(?:좀\s*더\s*)?따뜻하게\s*(?:해\s*줄?\s*수|할\s*수|해|해줘|해주세요|했으면)',
     '네, 난방 온도를 올리겠습니다.'),
    ...
    # 잠금 상태 확인 (잠금 명령보다 우선) — "현관문 잠갔어?" door_control dir=open 오분류 방지
    (r'현관문?\s*(?:잠갔|잠겼|잠금\s*됐|잠겨)(?:어|나|었나)?|도어락\s*(?:잠갔|잠겼)(?:어|나)?',
     '현재 현관 도어락이 잠겨 있습니다.'),
    ...
    (r'층간소음\s*(?:신고|민원|접수)|이웃\s*층간|소음\s*신고',
     '층간소음 민원은 경비실 또는 관리사무소에 신고해주세요. 층간소음 이웃사이센터(1661-2642)도 이용 가능합니다.'),
    ...
    # 서큘레이터 (heat_control 오분류 방지)
    (r'서큘레이터\s*(?:꺼|꺼줘|꺼주세요|끄|끄줘)',
     '네, 서큘레이터를 끕니다.'),
    (r'서큘레이터',
     '네, 서큘레이터를 켰습니다.'),
    # 난방 최고/최대/빨리 — heat_control dir=up 오분류 방지
    (r'(?:난방|보일러)\s*(?:최고|최대|최대한)\s*로?\s*(?:틀|켜|돌려|올려)',
     '네, 난방을 최대로 켭니다. 빠르게 따뜻해질 것입니다.'),
    (r'(?:난방|보일러)\s*(?:빨리|급히|빠르게)\s*(?:틀|켜|돌려|올려)',
     '네, 난방을 켭니다. 빠르게 따뜻해질 것입니다.'),
    # 에어컨 최저/최소 — ac_control dir=set + 숫자 없음
    (r'에어컨\s*(?:최저|최소|최저\s*온도|가장\s*낮은\s*온도)',
     '네, 에어컨을 최저 온도로 설정합니다.'),

    # 에어컨 끄고 환기 켜줘 (복합 명령 — NLU dir=on 오분류 방지)
    (r'에어컨\s*(?:끄|꺼)\s*(?:고|고\s*나서|고\s*나면)?\s*환기\s*(?:켜|켜줘|켜주세요)',
     '네, 에어컨을 끄고 환기 시스템을 켰습니다.'),
    # 실내 CO2 수치 쿼리 (탄소절감 패턴보다 먼저)
    (r'(?:실내|집\s*안)?\s*(?:이산화탄소|CO2|공기\s*중\s*이산화탄소)\s*(?:수치|농도|레벨|수준|알려|어때)',
     '현재 실내 CO2 농도는 OOppm입니다. 정상 범위(1000ppm 이하)입니다.'),
    # 정전/전기 이상 (에너지 쿼리 오분류 방지)
    (r'정전|전기\s*(?:나갔|꺼졌|끊겼|안\s*들어와)|정전\s*됐',
     '정전 시 한전 고객센터(123)에 문의하거나 관리사무소에 연락해주세요.'),
    # 전력 절약 요청 (에너지 off 오응답 방지)
    (r'전력\s*(?:사용량|절약|소비)\s*(?:절반|줄여|낮춰|절감)',
     '에너지 절약 모드를 설정하면 일부 기기가 자동 절전됩니다. 에너지 메뉴에서 설정해주세요.'),
    # 바깥 온도 쿼리
    (r'(?:바깥|밖|외부|야외)\s*(?:온도|기온)\s*(?:알려|어때|몇\s*도|어떻게)',
     '현재 OO구 OO동 기온은 OO도입니다.'),
    # 집 안 온도 쿼리 (indoor — weather_judge 오분류 방지)
    (r'집\s*(?:안|내부|실내)\s*온도\s*(?:몇\s*도|어때|알려|어떻게)',
     '현재 실내 온도는 23도입니다.'),
    # 적정 실내 습도 지식 쿼리
    (r'이상적인\s*(?:실내\s*)?습도|적정\s*(?:실내\s*)?습도|적절한\s*습도',
     '적정 실내 습도는 40~60%입니다. 현재 실내 습도는 OO%입니다.'),
    ...
    # 조명 색온도 (따뜻/차가운 색조) — "색온도 따뜻하게" 포함
    (r'조명\s*(?:색|색깔|색상|색온도)?\s*따뜻|색온도\s*따뜻|전구색\s*으로|따뜻한\s*빛',
     '네, 조명을 따뜻한 색온도로 설정합니다.'),
    ...
    # 실내 습도
    (r'실내\s*습도|습도\s*(?:어때|어떻|알려|확인|얼마|몇)|지금\s*습도',
     '현재 실내 습도는 OO%입니다.'),
    ...
    (r'일출\s*(?:몇\s*시|시간|언제)',
     '오늘 OO구 OO동 일출 시각은 오전 6시 00분입니다.'),
    (r'일몰\s*(?:몇\s*시|시간|언제)|해\s*지는\s*(?:시간|몇\s*시)',
     '오늘 OO구 OO동 일몰 시각은 오후 7시 00분입니다.'),
    (r'체감\s*(?:온도|기온)\s*(?:어때|몇\s*도|얼마)',
     '현재 OO구 OO동 체감온도는 OO도입니다.'),
    ...
    (r'수도\s*사용량|수도\s*요금',
     '이번 달 수도 사용량은 지난달 대비 OO% 수준입니다.'),
    (r'가스\s*(?:사용량|요금|얼마|나왔)|이번\s*달\s*가스',
...
def query_response(fn, room, raw_text):
    ...
    # Energy
    if fn == 'energy_query':
        if re.search(r'가스\s*(?:비|요금|비용)', raw_text):
            return '이번 달 가스 요금은 00000원 수준입니다. 자세한 내용은 관리비 메뉴에서 확인하세요.'
        if re.search(r'(?:수도|물)\s*(?:비|요금|비용)', raw_text):
            return '이번 달 수도 요금은 00000원 수준입니다. 자세한 내용은 관리비 메뉴에서 확인하세요.'
...
def control_response(fn, direction, room, value, raw_text, old_value=None):
    ...
    # 조명 밝기 모드 (은은하게/아늑하게/분위기/낭만/파티/공부/독서)
    if fn == 'light_control' and re.search(r'은은|아늑|분위기|영화\s*보기|간접|무드|낭만|로맨틱|파티|공부|독서|집중', raw_text):
        if re.search(r'낭만|로맨틱', raw_text):
            return f'네, {room_pref}조명을 따뜻하고 은은한 분위기로 설정합니다.'
        if re.search(r'파티', raw_text):
            return f'네, {room_pref}조명을 파티 모드로 설정합니다.'
        if re.search(r'공부|독서|집중', raw_text):
            return f'네, {room_pref}조명을 독서에 적합한 밝기로 설정합니다.'
...
def generate_response_v2(multihead, raw_text=''):
    ...
    if EMERGENCY_PATTERN.search(raw_text):
        if not re.search(
            ...
            # 요리/조리 맥락에서 연기/타는냄새 → 비상 아님 (환기 권장)
            if re.search(r'(?:요리|음식|밥|찌개|볶음|굽다|구워|조리)\s*.{0,10}(?:연기|냄새)|'
                         r'(?:연기|타는\s*냄새)\s*.{0,15}(?:요리|음식|밥|굽)', raw_text):
                return '요리 중 연기가 발생하면 환기를 권장합니다. 환기 시스템을 켜드릴까요?'
            return emergency_response()

    # 1.1 부정 명령 — "끄지 마", "켜면 안", "안 꺼도 돼" → 현재 상태 유지 (clarify/control 분기 전에 처리)
    if re.search(r'(?:끄|꺼|켜|열|닫|잠그)(?:지\s*말|지\s*마|면\s*안|지\s*않아)', raw_text):
        return '네, 현재 상태를 유지하겠습니다.'
    if re.search(r'안\s*(?:꺼도|켜도|열어도|닫아도|잠가도)\s*(?:돼|됩니다|돼요)', raw_text):
        return '네, 현재 상태를 유지하겠습니다.'
    if re.search(r'(?:에어컨|난방|조명|불|환기|커튼)\s*(?:필요\s*없어|안\s*해도\s*돼|괜찮아)', raw_text):
        return '알겠습니다. 필요하시면 언제든 말씀해주세요.'
    ...
    if room == 'none' and exec_t == 'control_then_confirm' and direction in ('up', 'down'):
        if fn in ('heat_control', 'ac_control') and not re.search(r'전체|모든|다|전부', raw_text):
            if not re.search(r'팬|풍속|풍량|바람\s*세|바람\s*속|속도|루버|방향|전기\s*(?:장판|매트|담요)', raw_text):
                if not re.search(r'빨리|급히|빠르게|최고|최대|최저', raw_text):
